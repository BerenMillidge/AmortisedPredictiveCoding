# testing whether the backprop still works WITHOUT the backward derivatives.
#if it does then that would be amazing as avoids another biological implausibility
...
class PredictiveCodingLayer(object):
  def __init__(self,input_size, output_size, batch_size, fn,fn_deriv,learning_rate,dropout_prob = None):
  ...
  def step(self,pe_below,pred,use_top_down_pe=True):
    ...
    self.mu+= self.learning_rate * (-pe + (np.dot(self.backwards_weights, pe_below)))
...
class AmortisedPredictiveCodingNetwork(object):
  def __init__(self, layer_sizes,batch_size,learning_rate,amortised_learning_rate, f,df,qf,dqf,n_inference_steps,dropout_prob = None):
    self.layer_sizes = layer_sizes
    self.batch_size = batch_size
    self.learning_rate = learning_rate
...
def run_amortised(trainset, valset):
    ...
    prednet = AmortisedPredictiveCodingNetwork(layer_sizes, batch_size,learning_rate,amortised_learning_rate, fn, fn_deriv,qf, qf_deriv,n_inference_steps,dropout_prob)
