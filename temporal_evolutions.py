#plotting/animating the evolution of the system through variational updating
...
class AmortisedPredictiveCodingNetwork(object):
  def __init__(self, layer_sizes,batch_size,learning_rate,amortised_learning_rate,n_inference_steps_train,n_inference_steps_test, f,df,qf,dqf,inference_threshold=0.1,save_imgs=False):
    self.layer_sizes = layer_sizes
    self.batch_size = batch_size
    self.learning_rate = learning_rate
  ...
  def forward_pass(self, img_batch, label_batch, test=False,save_imgs=False):
    # reset model
    ...
    if save_imgs:
        amortised_pred = self.generate_amortised(img_batch)
        pred_img_plots = []
        pred_label_plots = []
    if not test:
        self.layers[-1].mu = deepcopy(label_batch)
    ...
    for n in range(n_inference_steps):
        ...
        if save_imgs:
            pred_img_plots.append(deepcopy(self.predictions[0]))
            pred_label_plots.append(deepcopy(self.layers[-1].mu))
    ...
    if save_imgs:
        return pred_imgs, pred_labels, amortised_pred, np.array(pred_img_plots), np.array(pred_label_plots)
    else:
        return pred_imgs, pred_labels
  ...
  def test(self, img_batch, label_batch):
    return self.forward_pass(img_batch, label_batch,test=True)
  ...
  def train(self, imglist, labellist,test_img_list, test_label_list, n_epochs,save_name,anim_save_base):
    prediction_errors = []
    ...
    for n in range(n_epochs):
        ...
        if n % 1 == 0:
            ...
            if self.save_imgs:
                pred_imgs, pred_labels, amortised_pred, pred_img_plots, pred_label_plots = self.forward_pass(imglist[0],labellist[0],test=True,save_imgs = self.save_imgs)
                print(amortised_pred.shape, pred_img_plots.shape, pred_label_plots.shape)
                animate_label_beliefs(pred_label_plots,anim_save_base,n)
                animate_input_predictions(pred_img_plots,anim_save_base,n)
                for i in range(len(pred_img_plots)):
                    if i < 10:
                        img = pred_img_plots[i,:,0]
                        plt.imshow(img.reshape(28,28))
                        plt.show()
                        #plt.plot(pred_label_plots[i,:,0])
                        #print(pred_label_plots[i,:,0])
                        #plt.show()
...
def run_amortised(save_name,save_imgs,anim_save_base):
    batch_size = 1
    num_batches = 1
    num_test_batches = 2
    ...
    pred_net.train(img_list, label_list,test_img_list, test_label_list, n_epochs,save_name,anim_save_base)

if __name__ == "__main__":
    sname = str(sys.argv[1])
    save_imgs = str(sys.argv[2])
    anim_save_base = str(sys.argv[3])
    run_amortised(sname,save_imgs,anim_save_base)
