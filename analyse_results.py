...
def plot_error_dispersion_connections(standard_sname ="standard",error_dispersion_sname="error_dispersion_with_weight_updates",no_error_update_sname="proper_error_dispersion_without_weight_updates"):
    ...
    plt.rcParams['font.family'] = 'sans-serif'
    plt.rcParams['font.sans-serif'] = 'Arial'
    ax.fill_between(xs, standard_means - standard_stds,standard_means + standard_stds, alpha=0.1)
    ax.plot(xs, standard_means, label='Standard Predictive Coding')
    ax.fill_between(xs, error_means - error_stds, error_means + error_stds, alpha=0.1)
    ax.plot(xs, error_means, label="Error connections")
    ax.fill_between(xs, no_update_means - no_update_stds, no_update_means + no_update_stds, alpha=0.1)
    ax.plot(xs, no_update_means, label="No updates")
    ...
    plt.title("Error Dispersion Accuracy",fontsize=16)
...
def plot_nonlinearities(standard_sname ="standard",nonlinearities_sname="no_backward_nonlinearities"):
    ...
    ax.fill_between(xs, standard_means - standard_stds,standard_means + standard_stds, alpha=0.1)
    ax.plot(xs, standard_means, label='Standard Predictive Coding')
    ax.fill_between(xs, nonlinearities_means - nonlinearities_std, nonlinearities_means + nonlinearities_std, alpha=0.1)
    ax.plot(xs, nonlinearities_means, label="No Backward Nonlinearities")
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    #ax.spines['left'].set_smart_bounds(True)
    #ax.spines['bottom'].set_smart_bounds(True)
    plt.xlabel("Epoch",fontsize=12)
    plt.ylabel("Test Accuracy",fontsize=12)
    plt.title("No Backward Nonlinearities Accuracy",fontsize=16)
    leg = plt.legend(fontsize=12)
    frame = leg.get_frame()
    frame.set_facecolor('1.0')
    frame.set_edgecolor('1.0')
...
def plot_backwards_weights(standard_sname ="standard",backward_weight_sname="backward_weight_update",no_update_sname="no_backward_weight_update"):
    ...
    ax.fill_between(xs, standard_means - standard_stds,standard_means + standard_stds, alpha=0.1)
    ax.plot(xs, standard_means, label='Standard Predictive Coding')
    ax.fill_between(xs, backward_weight_means - backward_weight_stds, backward_weight_means + backward_weight_stds, alpha=0.1)
    ax.plot(xs, backward_weight_means, label="Backward Weights")
    ax.fill_between(xs, no_update_means - no_update_stds, no_update_means + no_update_stds, alpha=0.1)
    ax.plot(xs, no_update_means, label="No updates")
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ...
    plt.xlabel("Epoch",fontsize=12)
    plt.ylabel("Test Accuracy",fontsize=12)
    plt.title("Backward Weight Accuracy",fontsize=16)
    leg = plt.legend(fontsize=12)
    frame = leg.get_frame()
    frame.set_facecolor('1.0')
    frame.set_edgecolor('1.0')
