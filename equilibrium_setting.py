#experiments with chris' version of what's going on where the amortisation just sets the equilibrium, as I understand it
import numpy as np
...
class PredictiveCodingLayer(object):
  ...
  def update_mu(self, pe, pe_below):
    return self.learning_rate * (-pe + (np.dot(self.weights.T, pe_below *self.fn_deriv(np.dot(self.weights, self.mu)))))
...
class AmortisedPredictiveCodingNetwork(object):
  def __init__(self, layer_sizes,batch_size,learning_rate,amortised_learning_rate,n_inference_steps_train,n_inference_steps_test, f,df,qf,dqf,inference_threshold=0.1):
    ...
    self.mu_a = [[] for i in range(self.L)]
    self.mu_v = [[] for i in range(self.L)]

  def reset_mus(self):
    for (i,layer) in enumerate(self.layers):
        layer.mu = np.random.normal(0,1,[layer.output_size,layer.batch_size])
        self.mu_v[i] = np.zeros_like(layer.mu)
  ...
  def amortisation_pass(self, img_batch,initialize=True):
      ...
      for i in range(self.L):
          ...
          self.mu_a[i] = deepcopy(self.amortised_predictions[i+1])
  ...
  def forward_pass(self, img_batch, label_batch, test=False):
    ...
    for n in range(n_inference_steps):
        ...
        for i in range(self.L):
            ...
            if i != self.L-1:
                mu_v_grad = self.layers[i].update_mu(self.prediction_errors[i+1], self.prediction_errors[i])
                self.mu_v[i] += mu_v_grad
                self.layers[i].mu = self.mu_a[i] + self.mu_v[i]
            else:
                mu_v_grad = self.layers[i].update_mu(np.zeros_like(self.layers[i].mu), self.prediction_errors[i])
                self.mu_v[i] += mu_v_grad
                self.layers[i].mu = self.mu_a[i] + self.mu_v[i]
            self.predictions[i] = self.layers[i].predict()
...
if __name__ == "__main__":
    sname = str(sys.argv[1]) if len(sys.argv)>=2 else None
    run_amortised(sname)
